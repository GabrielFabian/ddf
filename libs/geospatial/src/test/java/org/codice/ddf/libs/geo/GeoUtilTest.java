...
public class GeoUtilTest {
  ...
  private static final int DEFAULT_MAX_VERTICIES = 32;
  ...
  public void testConvertPointRadiusToCirclePolygon() throws ParseException {
    ...
    Geometry circle =
        GeospatialUtil.createCirclePolygon(
            lat, lon, radius, DEFAULT_MAX_VERTICIES, DEFAULT_DISTANCE_TOLERANCE);
    Assert.assertThat(
        circle.getCoordinates().length,
        Is.is(equalTo(DEFAULT_MAX_VERTICIES + 1))); // additional coordinate for closing the polygon
    Assert.assertThat(circle, Is.is(equalTo(expectedCircle)));
  }

  @Test
  public void testReduceVerticies() {
    double lat = 43.25;
    double lon = -123.45;
    double radius = 500000;
    int maxVerticies = 10;

    Geometry circle = GeospatialUtil.createCirclePolygon(lat, lon, radius, maxVerticies, 0.5);

    Assert.assertThat(circle.getCoordinates().length, Is.is(lessThan(maxVerticies + 1)));
  }
}
